...
export function exportTasksToCSV(tasks: TaskData[], projectName: string) {
  const headers = ['ID', 'Title', 'Description', 'Priority', 'Is Manual', 'Source Group'];
...
export async function exportDiagramToPNG(htmlElement: HTMLElement, title: string) {
  try {
    const dataUrl = await toPng(htmlElement, { quality: 1, backgroundColor: '#ffffff' });
    const a = document.createElement('a');
    a.href = dataUrl;
...
export function compileProjectToMarkdown(
  ...
  for (const node of sortedNodes) {
    ...
    if (node.type === 'project_brief' && c.structured_fields) {
      ...
      if (sf.objectives?.length) md += `**Objectives:**\n${sf.objectives.map((o: string) => `- ${o}`).join('\n')}\n\n`;
      if (sf.target_users?.length) md += `**Target Users:** ${sf.target_users.join(', ')}\n\n`;
      if (sf.scope_in?.length) md += `**Scope In:**\n${sf.scope_in.map((s: string) => `- ${s}`).join('\n')}\n\n`;
      if (sf.scope_out?.length) md += `**Scope Out:**\n${sf.scope_out.map((s: string) => `- ${s}`).join('\n')}\n\n`;
      if (sf.success_metrics?.length) md += `**Success Metrics:**\n${sf.success_metrics.map((m: any) => `- ${m.metric}: ${m.target}`).join('\n')}\n\n`;
      if (sf.constraints?.length) md += `**Constraints:**\n${sf.constraints.map((c: string) => `- ${c}`).join('\n')}\n\n`;
      if (sf.tech_stack?.length) md += `**Tech Stack:** ${sf.tech_stack.join(', ')}\n\n`;
      if (sf.references?.length) md += `**References:**\n${sf.references.map((r: any) => `- [${r.name}](${r.url})`).join('\n')}\n\n`;
    }
    ...
    if (node.type === 'use_cases' && c.structured_fields?.cases) {
      c.structured_fields.cases.forEach((uc: any) => {
        md += `### ${uc.name}\n`;
        md += `- **Actors:** ${uc.actors?.join(', ') || 'System'}\n`;
...
export function exportProjectToMarkdown(project: Project, nodes: NodeData[], contents: NodeContent[]) {
  const md = compileProjectToMarkdown(project, nodes, contents);
  downloadFile(md, `${project.name}-Documentation.md`, 'text/markdown');
}

export function exportProjectToPDF(project: Project, nodes: NodeData[], contents: NodeContent[]) {
  const md = compileProjectToMarkdown(project, nodes, contents);
