export function generateMermaid(nodeType: string, fields: Record<string, any>): string {
  try {
    switch (nodeType) {
      case 'use_cases': return generateUseCaseMermaid(fields);
      case 'erd': return generateERDMermaid(fields);
      case 'sequence': return generateSequenceMermaid(fields);
      case 'flowchart': return generateFlowchartMermaid(fields);
      case 'dfd': return generateDFDMermaid(fields);
      case 'project_brief': return "mindmap\n  root((Project))\n    Background\n    Scope\n    Metrics";
      case 'requirements': return "mindmap\n  root((Requirements))\n    Functional\n    NonFunctional";
      case 'user_stories': return "mindmap\n  root((User Stories))";
      default: return "";
    }
  } catch (e) {
...
function generateUseCaseMermaid(fields: any): string {
  ...
  if (actors.length === 0 && useCases.length === 0) return "flowchart TD\n  Start(Start)";
...
function generateFlowchartMermaid(fields: any): string {
  ...
  if (flows.length === 0 && legacySteps.length === 0) return "flowchart TD\n  Start(Start)";
  ...
  const renderStepsAndConns = (steps: any[], conns: any[]) => {
    steps.forEach((s: any) => {
      ...
      else if (s.type === 'start' || s.type === 'end') m += `  ${id}(["${lbl}"])\n`;
      else m += `  ${id}["${lbl}"]\n`;
    });
    conns.forEach((c: any) => {
  ...
  if (flows.length > 0) {
    flows.forEach((flow: any, idx: number) => {
      if (flows.length > 1) m += `  subgraph ${flow.name || `Flow_${idx + 1}`}\n`;
