import { db, NodeData, NodeContent, ValidationWarning } from './db';
...
export function validateNode(nodeType: string, fields: any): LocalValidationIssue[] {
  const issues: LocalValidationIssue[] = [];
  
  if (!fields) return issues;

  if (nodeType === 'project_brief') {
    if (!fields.name) issues.push({ field: 'name', message: 'Project Name is missing.' });
    if (!fields.background) issues.push({ field: 'background', message: 'Background / Why is missing.' });
    if (!fields.objectives || fields.objectives.length === 0) issues.push({ field: 'objectives', message: 'At least one objective is required.' });
    if (!fields.target_users || fields.target_users.length === 0) issues.push({ field: 'target_users', message: 'At least one target user is required.' });
    if (!fields.scope_in || fields.scope_in.length === 0) issues.push({ field: 'scope_in', message: 'At least one Scope In item is required.' });
    if (!fields.scope_out || fields.scope_out.length === 0) issues.push({ field: 'scope_out', message: 'At least one Scope Out item is required.' });
    if (!fields.success_metrics || fields.success_metrics.length === 0) issues.push({ field: 'success_metrics', message: 'At least one success metric is required.' });
  }
  ...
  if (nodeType === 'requirements') {
    ...
    if (frs.length < 3) issues.push({ field: 'items', message: `Min 3 Functional Requirements required (currently ${frs.length}).` });
    if (nfrs.length < 1) issues.push({ field: 'items', message: 'At least 1 Non-Functional Requirement is required.' });
    if (!items.some((i: any) => i.priority === 'Must')) issues.push({ field: 'items', message: 'At least 1 requirement must have "Must" priority.' });
  }
  
  if (nodeType === 'user_stories') {
    if (!fields.items || fields.items.length === 0) issues.push({ field: 'items', message: 'No user stories defined.' });
    else {
      fields.items.forEach((st: any, i: number) => {
        if (!st.role || !st.goal) issues.push({ field: `items[${i}]`, message: `Story #${i+1} is missing role or goal.` });
      });
    }
  }

  if (nodeType === 'erd') {
    if (!fields.entities || fields.entities.length === 0) issues.push({ field: 'entities', message: 'No entities defined.' });
  }
...
export async function crossValidateAll(projectId: string): Promise<void> {
  ...
  const contents = await db.nodeContents.where('node_id').anyOf(nodes.map(n => n.id)).toArray();
  ...
  // Helper arrays
  const frItems = (reqs?.fields.items || []).filter((i: any) => (i.type || 'FR') === 'FR');
  ...
  const seqMessages = sequence?.fields.messages || [];

  // Existing tasks for CV-16/17
  const allTasks = taskBoard ? await db.tasks.where({ project_id: projectId }).toArray() : [];
  ...
  if (brief && brief.fields.target_users?.length > 0 && stories) {
    const storyRoles = storyItems.map((st: any) => (st.role || '').toLowerCase());
  ...
  if (brief && brief.fields.target_users?.length > 0 && useCases) {
    const ucActors = (useCases.fields.actors || []).map((a: string) => (a || '').toLowerCase());
    brief.fields.target_users.forEach((user: string) => {
  ...
  if (brief && brief.fields.scope_in?.length > 0 && reqs) {
    const coveredScopes = new Set(frItems.map((i: any) => (i.related_scope || '').toLowerCase()).filter(Boolean));
  ...
  if (reqs && stories) {
    const storyReqs = new Set(storyItems.map((s: any) => (s.related_requirement || '').toLowerCase()).filter(Boolean));
    frItems.forEach((fr: any, idx: number) => {
      const frId = `FR-${String(idx + 1).padStart(3, '0')}`;
      if (!storyReqs.has(fr.id?.toLowerCase()) && !storyReqs.has(frId.toLowerCase())) {
        warnings.push({
          project_id: projectId,
          source_node_id: reqs.node.id,
          target_node_type: 'user_stories',
  ...
  if (stories && useCases && ucItems.length > 0) {
    ...
    ucItems.forEach((uc: any) => {
      (uc.related_stories || []).forEach((storyId: string) => ucRelatedStories.add(storyId));
    });
  ...
  if (useCases && flowchart && ucItems.length > 0) {
    const flowRelatedUCs = new Set(flowchartFlows.map((f: any) => f.related_use_case).filter(Boolean));
  ...
  if (dfd && brief && brief.fields.target_users?.length > 0) {
    const targetUsersLower = brief.fields.target_users.map((u: string) => u.toLowerCase());
  ...
  if (erd && erdEntities.length > 0 && (stories || useCases)) {
    const allText = [
      ...
      ...ucItems.map((uc: any) => `${uc.name || ''} ${(uc.main_flow || []).map((s: any) => s.action || '').join(' ')}`),
    ].join(' ').toLowerCase();
    erdEntities.forEach((e: any) => {
      const name = (e.name || '').toLowerCase();
  ...
  if (sequence && dfd) {
    const dfdExtNames = dfdExtEntities.map((n: any) => (n.label || '').toLowerCase());
    seqParticipants.forEach((p: any) => {
      const name = (typeof p === 'string' ? p : p?.name || '').toLowerCase();
      const type = typeof p === 'string' ? 'component' : (p?.type || 'component');
  ...
  if (stories && storyItems.length > 0 && allTasks.length > 0) {
    const taskSources = new Set(allTasks.map(t => t.source_item_id).filter(Boolean));
  ...
  if (useCases && ucItems.length > 0) {
    ...
    ucItems.forEach((uc: any, idx: number) => {
      (uc.relationships || []).forEach((rel: any) => {
        if (rel.target && !ucIds.has(rel.target)) {
          warnings.push({
            project_id: projectId,
            source_node_id: useCases.node.id,
            target_node_type: 'use_cases',
