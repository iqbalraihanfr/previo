...
const FR_CATEGORIES = [
  'Authentication', 'Authorization', 'User Management', 'Data Management',
  'Reporting', 'Notification', 'Payment', 'Integration', 'Search',
  'File Management', 'Communication', 'Settings', 'Dashboard',
];
...
export function RequirementEditor({ fields, onChange, projectId }: EditorProps) {
    ...
    <div className="flex flex-col h-full w-full">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="w-full">
            <TabsTrigger value="FR" className="flex-1 text-xs">
              Functional (FR) {frCount > 0 && <span className="ml-1 text-[10px] bg-primary/10 text-primary px-1.5 rounded-full">{frCount}</span>}
            </TabsTrigger>
            <TabsTrigger value="NFR" className="flex-1 text-xs">
              Non-Functional (NFR) {nfrCount > 0 && <span className="ml-1 text-[10px] bg-primary/10 text-primary px-1.5 rounded-full">{nfrCount}</span>}
            </TabsTrigger>
          </TabsList>
        </Tabs>
      ...
      <div className="p-4 overflow-y-auto flex-1 flex flex-col gap-4">
        <div className="flex items-center justify-between shrink-0 mb-2">
          <Label className="text-sm font-semibold">{activeTab === 'FR' ? 'Functional Requirements' : 'Non-Functional Requirements'}</Label>
        ...
        <div className="space-y-4 pb-4">
          {activeItems.map((item: any) => (
            <div key={item.id} className="flex flex-col gap-3 p-4 border rounded-md bg-background shadow-sm relative">
              ...
              <div className="grid gap-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => updateItem(item.id, { description: e.target.value.slice(0, 200) })}
                  placeholder="Requirement details..."
                  className="min-h-[60px] resize-none text-sm"
                  maxLength={200}
                />
                <p className="text-[10px] text-muted-foreground text-right">{(item.description || '').length}/200</p>
              </div>
              ...
              {activeTab === 'FR' && scopeInItems.length > 0 && (
                <div className="grid gap-2">
                  <Label className="text-xs text-muted-foreground">Related Scope Item</Label>
                  <Select
                    value={item.related_scope || ''}
                    onValueChange={(val) => updateItem(item.id, { related_scope: val === '__none__' ? '' : val })}
                  >
                    <SelectTrigger className="h-8 text-sm">
                    ...
                    <SelectContent>
                      ...
                      {scopeInItems.filter(Boolean).map((scope: string, idx: number) => (
                        <SelectItem key={idx} value={scope}>{scope}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
              ...
              {activeTab === 'NFR' && (
                <div className="flex items-end gap-3">
                  <div className="grid gap-2 flex-1">
                    <Label className="text-xs text-muted-foreground">Metric <span className="text-destructive">*</span></Label>
                    <Input
                      value={item.metric || ''}
                      onChange={(e) => updateItem(item.id, { metric: e.target.value })}
                      placeholder="e.g. Response Time"
                      className="h-8 text-sm"
                    />
                  </div>
                  <div className="grid gap-2 flex-1">
                    <Label className="text-xs text-muted-foreground">Target <span className="text-destructive">*</span></Label>
                    <Input
                      value={item.target || ''}
