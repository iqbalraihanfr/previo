...
import { EditorProps, type StructuredEditorFields } from "./ProjectBriefEditor";
...
type ERDFields = StructuredEditorFields & {
  entities?: ERDEntity[];
...
export function ERDEditor({ fields, onChange }: EditorProps<ERDFields>) {
  const entities = Array.isArray(fields.entities) ? fields.entities : [];
  const relationships = Array.isArray(fields.relationships)
    ? fields.relationships
    : [];

  const updateEntities = (newE: ERDEntity[]) =>
    onChange({ ...fields, entities: newE });
  const updateRels = (newR: ERDRelationship[]) =>
    onChange({ ...fields, relationships: newR });
  ...
  const addAttribute = (entityId: string) => {
    const e = entities.find((entity) => entity.id === entityId);
    if (!e) return;
  ...
  const removeAttribute = (entityId: string, idx: number) => {
    const e = entities.find((entity) => entity.id === entityId);
    if (!e) return;
    const attrs = (Array.isArray(e.attributes) ? e.attributes : []).filter(
      (_, i: number) => i !== idx,
    );
    updateEntity(entityId, { attributes: attrs });
  };
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-4">
          {entities.map((ent) => (
            <div
              key={ent.id}
              className="flex flex-col gap-3 p-3 border rounded-md bg-background relative shadow-sm"
            >
              <div className="flex items-center justify-between">
              ...
              <div className="grid gap-2">
                <Label className="text-xs text-muted-foreground">
                  Name (UPPERCASE) <span className="text-destructive">*</span>
                </Label>
                <Input
                  value={ent.name || ""}
                  onChange={(e) =>
                    updateEntity(ent.id, {
                      name: formatEntityName(e.target.value),
                    })
                  }
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateEntity(ent.id, { description: e.target.value })
                  }
              ...
              <div className="mt-2 space-y-2 border-t pt-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={() => addAttribute(ent.id)}
                  >
                    <Plus className="h-3 w-3 mr-1" /> Add
                  </Button>
                </div>
                <div className="space-y-2">
                  {(Array.isArray(ent.attributes) ? ent.attributes : []).map(
                    (attr, idx: number) => (
                      <div
                        key={idx}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <Input
                            value={attr.name}
                            onChange={(e) =>
                              updateAttribute(ent.id, idx, {
                                name: formatAttrName(e.target.value),
                              })
                            }
                          ...
                          <Button
                            ...
                            onClick={() => removeAttribute(ent.id, idx)}
                          >
                        ...
                        <div className="flex flex-wrap gap-3 text-[10px]">
                          ...
                          <label className="flex items-center gap-1 cursor-pointer">
                            <input
                              type="checkbox"
                              checked={attr.isIndex || false}
                              onChange={(e) =>
                                updateAttribute(ent.id, idx, {
                                  isIndex: e.target.checked,
                                })
                              }
                            />{" "}
                            Index
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          {relationships.map((rel) => (
            <div
              key={rel.id}
              className="p-3 border rounded-md bg-background shadow-sm space-y-2"
            >
              <div className="flex items-center gap-2">
                <select
                  value={rel.from}
                  onChange={(e) => updateRel(rel.id, { from: e.target.value })}
                  className="h-8 text-xs border rounded bg-background px-1 flex-1 truncate"
                >
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateRel(rel.id, {
                      type: e.target.value as ERDRelationship["type"],
                    })
                  }
                  className="h-8 text-xs border rounded bg-background px-1 w-28"
                >
                ...
                <select
                  value={rel.to}
                  onChange={(e) => updateRel(rel.id, { to: e.target.value })}
                  className="h-8 text-xs border rounded bg-background px-1 flex-1 truncate"
                >
              ...
              <Input
                value={rel.label || ""}
                onChange={(e) => updateRel(rel.id, { label: e.target.value })}
                placeholder="Relationship verb (e.g. places, contains, belongs_to)"
                className="h-7 text-xs"
              />
              ...
              {rel.type === "many-to-many" && rel.from && rel.to && (
                <div className="flex items-center gap-2 text-xs p-2 bg-amber-500/10 border border-amber-500/20 rounded">
                  ...
                  <span>
                    N:M detected. Suggested junction table:{" "}
                    <strong>
                      {rel.junction_table || `${rel.from}_${rel.to}`}
                    </strong>
                  </span>
                  <Input
                    value={rel.junction_table || `${rel.from}_${rel.to}`}
                    onChange={(e) =>
                      updateRel(rel.id, { junction_table: e.target.value })
                    }
