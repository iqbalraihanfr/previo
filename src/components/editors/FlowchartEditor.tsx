...
export function FlowchartEditor({ fields, onChange, projectId }: EditorProps) {
  const useCases = useUseCases(projectId) as any[];
  const flows = (fields.flows as any[]) || [];

  const updateFlows = (newFlows: any[]) =>
  ...
  const updateFlow = (id: string, updates: any) => {
    updateFlows(
      flows.map((f: any) => (f.id === id ? { ...f, ...updates } : f)),
    );
  };
  ...
  const addStep = (flowId: string) => {
    ...
    updateFlow(flowId, {
      steps: [
        ...(flow.steps || []),
        { id: crypto.randomUUID(), label: "", type: "process" },
      ],
    });
  };
  ...
  const removeStep = (flowId: string, stepId: string) => {
    ...
    if (!flow) return;
    updateFlow(flowId, {
      steps: (flow.steps || []).filter((s: any) => s.id !== stepId),
    });
  };
  ...
  const updateConnection = (flowId: string, connId: string, updates: any) => {
    ...
    if (!flow) return;
    updateFlow(flowId, {
      connections: (flow.connections || []).map((c: any) =>
  ...
  const removeConnection = (flowId: string, connId: string) => {
    ...
    if (!flow) return;
    updateFlow(flowId, {
      connections: (flow.connections || []).filter((c: any) => c.id !== connId),
    });
  };
  ...
  if (!fields.flows && (fields.steps?.length || fields.connections?.length)) {
    ...
    return (
      <div className="p-4 text-sm text-muted-foreground">
        Migrating to multi-flow format...
      </div>
    );
  }
      ...
      {flows.map((flow: any, flowIdx: number) => (
        ...
        >
          ...
          <div className="p-3 space-y-3">
            ...
            <div className="grid gap-2">
              <Label className="text-xs text-muted-foreground">
                Related Use Case <span className="text-destructive">*</span>
              </Label>
              <select
                value={flow.related_use_case || ""}
                onChange={(e) =>
                  updateFlow(flow.id, { related_use_case: e.target.value })
                }
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                placeholder="e.g. User clicks Register button"
                className="h-8 text-sm"
              />
            </div>
            ...
            <div className="space-y-2 border-t pt-3">
              <div className="flex items-center justify-between">
                <Label className="text-xs text-muted-foreground font-semibold uppercase">
                  ...
                  <span className="text-[10px] font-normal normal-case">
                    (min 4: start + process/decision + end)
                  </span>
                </Label>
                <Button
                  size="sm"
              ...
              <div className="space-y-1.5">
                {(flow.steps || []).map((step: any) => (
                  <div
                    key={step.id}
                    className="flex gap-2 items-center p-1.5 border rounded bg-muted/30"
                  >
                    <select
                      value={step.type}
                      onChange={(e) =>
                        updateStep(flow.id, step.id, { type: e.target.value })
                      }
                      className="h-7 text-[10px] border rounded bg-background px-1 w-20"
                    >
                    ...
                    <Input
                      value={step.label || ""}
                      onChange={(e) =>
                        updateStep(flow.id, step.id, { label: e.target.value })
                      }
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateStep(flow.id, step.id, {
                              yes_target: e.target.value,
                            })
                          }
                          placeholder="Yes→"
                          className="h-7 text-[10px] w-16 bg-green-50 dark:bg-green-950/20"
                          title="Yes path target step label"
                        />
                        <Input
                          value={step.no_target || ""}
                          onChange={(e) =>
                            updateStep(flow.id, step.id, {
                              no_target: e.target.value,
                            })
                          }
                          placeholder="No→"
                          className="h-7 text-[10px] w-16 bg-red-50 dark:bg-red-950/20"
                          title="No path target step label"
                        />
                      </>
                    )}
                    <Button
            ...
            <div className="space-y-2 border-t pt-3">
              ...
              <div className="space-y-1.5">
                {(flow.connections || []).map((conn: any) => (
                  <div
                    key={conn.id}
                    className="flex items-center gap-2 p-1.5 border rounded bg-muted/30"
                  >
                    <select
                      value={conn.from || ""}
                      onChange={(e) =>
                        updateConnection(flow.id, conn.id, {
                          from: e.target.value,
                        })
                      }
                      className="h-7 text-[10px] border rounded bg-background px-1 flex-1"
                    >
                      <option value="">From...</option>
                      {(flow.steps || []).map((s: any) => (
                        <option key={s.id} value={s.id}>
                          {s.label || s.id.substring(0, 6)}
                        </option>
                      ))}
                    </select>
                    ...
                    <Button
                      ...
                      size="icon"
                      className="h-7 w-7 text-muted-foreground hover:text-destructive shrink-0"
                      onClick={() => removeConnection(flow.id, conn.id)}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  </div>
